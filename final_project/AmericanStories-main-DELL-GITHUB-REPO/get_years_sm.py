...
def download_entire_dataset_by_year(year_value):
    """
    ...
    # base_output_dir = "./final_project/AmericanStories-main-DELL-GITHUB-REPO/data_by_year/"
...
def process_year(year):
    ...
    if os.path.exists(output_file_parquet) or os.path.exists(output_file_feather):
        print(f"File already exists for year {year}, skipping...")
        return
    
    start_time = datetime.now()
    print(f"Start downloading data for year {year} at {start_time}")
    
    try:
        download_entire_dataset_by_year(year)
        end_time = datetime.now()
        print(f"done: {year} at {end_time}, duration: {end_time - start_time}")
