...
baltcity_race %>% 
  summarize(
    ...
    pacific_islander_total = sum(x2020_pac_islander, na.rm = TRUE)
...
baltcity_income %>%
  select(Neighborhood, x2020) %>% 
  filter(x2020 < mean(x2020, na.rm=TRUE)) %>%
  summarise(
    below_average_tracts_count = n(),
    total_tracts_count = nrow(baltcity_income),
    percent_below_avg = (n()/nrow(baltcity_income)*100 
  ))
